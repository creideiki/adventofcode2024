...
class Disk
  ...
  def find_first_free_index
    @runs.index { |r| r.type == :empty }
  end

  def find_last_file_index
    @runs.rindex { |r| r.type == :file }
  end

  def compact_one_run!(free_index, file_index)
    free = @runs[free_index]
    file = @runs[file_index]
    size = [free.length, file.length].min
    ...
    if file.length.zero?
      @runs.delete_at(file_index)
    end
  end

  def compact!
    loop do
      free_index = find_first_free_index
      file_index = find_last_file_index

      return if file_index < free_index
